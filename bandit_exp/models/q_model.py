...
class ActionSoftMaxModel(Model):
    def __init__(self):
        """This class implements a simple action learning model."""
        self.is_fitted = False  # A flag to indicate whether the model is fitted or not.
    ...
    def fit(
            self, num_choices: int, actions: array_like, your_actions: array_like, **kwargs: dict
    ) -> None:
        """
        Fit the model to data using the maximum likelihood estimation (MLE).
        ...
        def neg_ll(args):
            a, b = args
            return self.calculate_nll(a, b, num_choices, actions, your_actions)
    ...
    def calculate_nll(
        ...
        float
            The negative log-likelihood
        """
        if len(actions) != len(your_actions):
            raise ValueError("The sizes of `actions` and `your_actions` must be the same.")
        if max(actions) > num_choices:
            raise ValueError("The range of `actions` exceeds `num_choices`.")

        time_horizon = len(actions)
        q_vals = np.ones((time_horizon, num_choices)) / 2

        # Update the estimated values, which are called Q values.
        # TODO: There must be numpy way
        for t in range(time_horizon - 1):
            prev_q_val = q_vals[t, actions[t]]
            q_vals[t + 1, actions[t]] = prev_q_val + learning_rate * (1 - prev_q_val)
            q_vals[t + 1, 1 - actions[t]] = 1 - q_vals[t + 1, actions[t]]

        choice_probs = softmax(inverse_temperature * q_vals, axis=1)
        neg_log_likelihood = - np.log(choice_probs[np.arange(time_horizon), your_actions] + 1e-8).sum()
        return neg_log_likelihood


class SocialQLearningSoftmax(Model):
    def __init__(self):
        """This class implements a simple action learning model."""
        self.is_fitted = False  # A flag to indicate whether the model is fitted or not.
    ...
    def fit(
            self, num_choices: int, actions: array_like, rewards: array_like, your_actions: array_like, **kwargs: dict
    ) -> None:
        """
        Fit the model to data using the maximum likelihood estimation (MLE).
        ...
        def neg_ll(args):
            a, b = args
            return self.calculate_nll(a, b, num_choices, actions, rewards, your_actions)
    ...
    def calculate_nll(
            ...
            inverse_temperature: float,
            num_choices: int,
            observed_actions: array_like,
            observed_rewards: array_like,
            your_actions: array_like,
    ) -> float:
        """
        ...
        float
            The negative log-likelihood
        """
        if len(observed_actions) != len(your_actions):
            raise ValueError("The sizes of `actions` and `your_actions` must be the same.")
        if max(observed_actions) > num_choices:
            raise ValueError("The range of `actions` exceeds `num_choices`.")

        time_horizon = len(observed_actions)
        q_vals = np.ones((time_horizon, num_choices)) / 2
        ...
        for t in range(time_horizon - 1):
            q_vals[t + 1, :] = q_vals[t, :]
            prev_q_val = q_vals[t, observed_actions[t]]
            q_vals[t + 1, observed_actions[t]] = prev_q_val + learning_rate * (observed_rewards[t] - prev_q_val)
